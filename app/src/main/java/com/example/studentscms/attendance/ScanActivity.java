...
public class ScanActivity extends AppCompatActivity {
    ...
    private ListenableFuture cameraProviderFutuer;
    private ExecutorService cameraExecutor;
    private PreviewView previewView;
    private MyImageAnyliser anyliser;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cameraProviderFutuer = ProcessCameraProvider.getInstance(this);

        anyliser = new MyImageAnyliser(getSupportFragmentManager());

        cameraProviderFutuer.addListener(new Runnable() {
            @Override
            public void run() {
                try {
                    if (ActivityCompat.checkSelfPermission(ScanActivity.this, Manifest.permission.CAMERA) != (PackageManager.PERMISSION_GRANTED)) {
                        ...
                        ProcessCameraProvider processCameraProvider = (ProcessCameraProvider) cameraProviderFutuer.get();
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull @NotNull String[] permissions, @NonNull @NotNull int[] grantResults) {
        ...
        if (requestCode == 101 && grantResults.length > 0) {
            ...
            try {
                processCameraProvider = (ProcessCameraProvider) cameraProviderFutuer.get();
            } catch (ExecutionException e) {
                e.printStackTrace();
            } catch (InterruptedException e) {
                e.printStackTrace();
    ...
    private void bindpreview(ProcessCameraProvider processCameraProvider) {
        ...
        ImageAnalysis imageAnalysis = new ImageAnalysis.Builder()
                ...
                .build();
        imageAnalysis.setAnalyzer(cameraExecutor, anyliser);
